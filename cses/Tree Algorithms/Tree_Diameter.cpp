...
int n;
vector<int>* adj;
pair<int,int> dfs(int nd, int p = -1){
    //{diameter, dpth}
    int dpth[3] = {};
    int mxdia = 0;
    for(auto& child : adj[nd]){
        if(child != p){
            pair<int,int> temp = dfs(child, nd);
            mxdia = max(mxdia, temp.first);
            dpth[0] = temp.second+1;
            sort(dpth, dpth+3);
        }
    }
